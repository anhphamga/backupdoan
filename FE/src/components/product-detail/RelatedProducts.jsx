  ...
  return (
    <section className="rounded-3xl border border-neutral-200 bg-white p-6 shadow-sm">
      <div className="mb-4 flex items-center justify-between">
        <h3 className="text-lg font-semibold text-neutral-900">Sản phẩm liên quan</h3>
      </div>
        ...
        <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
          {(items.length ? items : []).map((item) => (
            ...
            >
              ...
              <div className="p-4">
                <p className="min-h-[44px] text-sm font-semibold leading-5 text-neutral-900">{item.name || "Sản phẩm"}</p>
                <p className="mt-2 text-xs uppercase tracking-wide text-neutral-500">{item.category || "Danh mục"}</p>
              </div>
            </Link>
          ))}
