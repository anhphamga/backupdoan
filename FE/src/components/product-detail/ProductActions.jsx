  ...
  return (
    <>
      <div className="hidden rounded-3xl border border-neutral-200 bg-white p-4 md:block lg:p-5">
        <p className="text-sm text-neutral-500">Chọn màu/size trước khi đặt</p>
        <div className="mt-4 grid gap-4 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-end">
          <div className="min-w-0">
            <p className="text-[11px] font-medium uppercase tracking-[0.2em] text-neutral-500">Giá thuê</p>
            <p className="mt-1 whitespace-nowrap text-3xl font-bold text-amber-700">{rentPriceText}</p>
            <p className="mt-1 whitespace-nowrap text-sm text-neutral-500">Giá bán: {salePriceText}</p>
          </div>
      ...
      <div className="fixed inset-x-0 bottom-0 z-40 border-t border-neutral-200 bg-white/95 p-3 backdrop-blur md:hidden">
        <div className="mx-auto flex w-full max-w-3xl items-center gap-2">
          <div className="min-w-0 flex-1">
            ...
            <p className="whitespace-nowrap text-lg font-bold text-amber-700">{rentPriceText}</p>
          </div>
          <button
            type="button"
            onClick={onBuy}
